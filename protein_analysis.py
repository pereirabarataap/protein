...
def protein_correlation_clustering_go_count(
    ...
    protein_gos = {
        protein: {go[2] for go in record_protein_gos[protein]} for protein in proteins
    }
    ...
    axs[0].set_title(
        "Protein hierarchical clustering on protein correlations via ward-linkage"
    )
    ...
    fig, axs = plt.subplots(
        1, best_n_clusters, dpi=100, figsize=(7 * best_n_clusters, 5), sharey=True
    )
    all_gos = set()
    ...
    for cluster in cluster_labels:
        ...
        for protein in cluster_proteins[cluster]:
            cluster_specific_gos[cluster] = cluster_specific_gos[cluster].union(
                protein_gos[protein]
            )
        for other_cluster in cluster_labels:
            if cluster != other_cluster:
                for protein in cluster_proteins[other_cluster]:
                    cluster_specific_gos[cluster] = cluster_specific_gos[
                        cluster
                    ].difference(protein_gos[protein])
    ...
    for cluster in cluster_labels:
        ...
        for protein in cluster_proteins[cluster]:
            cluster_specific_protein_gos[cluster][protein] = cluster_specific_gos[
                cluster
            ].intersection(protein_gos[protein])

    cluster_specific_protein_gos_df = pd.DataFrame(cluster_specific_protein_gos)
    for cluster in cluster_labels:
        sb.countplot(
            x=cluster_specific_protein_gos_df[cluster]
            .dropna()
        ...
        axs[cluster].set_title(
            ...
            + str(cluster)
            + " with "
            + str(len(cluster_specific_gos[cluster]))
            + " cluster-specific GOs and"
            + str(len(cluster_proteins[cluster]))
            + " proteins"
        )
    axs[0].set_ylabel("Number of proteins")
    plt.suptitle(
        "How many proteins (y-axis) have this many GOs (x-axis) from the pool of cluster-specific GOs",
        fontsize=15,
    )
...
def mock_data(rows=100, cols=10):
    toy_cols = ["P98160", "O00468-6", "Q15149-4", "P08238", "P06733"]
    df = pd.DataFrame(
        np.random.randint(0, 1e9, size=(rows, len(toy_cols))), columns=toy_cols
    )
    return df


def get_data():
    # df = pd.read_csv("data/protein_gos.csv")
    # df = pd.read_csv("data/protein_gos_filtered.csv")
    df = pd.read_csv("data/protein_gos_filtered_2.csv")
    df = df.dropna()
    return df
